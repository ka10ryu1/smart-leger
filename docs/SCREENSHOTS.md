...
月・収支・カテゴリ・分類元・加盟店名・確信度で絞り込み。confidence が低い明細・Jev エラー・未分類の行には色が付きます。
内訳のある明細は下に内訳行を表示し、カテゴリの絞り込みは内訳のカテゴリにも一致します。
「手動で追加」から、現金での支出や CSV を取り込んでいないカードの支出を手入力で追加できます。
...
手動で追加した明細だけは「この明細を削除」で削除できます(取込明細は取込履歴から取込ごと取り消します)。
