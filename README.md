...
- **「ポート 5000 は使用中ですが Smart Ledger の応答を確認できませんでした」と出る**: 別のプログラムが同じポートを使っているか、応答しなくなった Smart Ledger のウィンドウが残っています。該当ウィンドウを閉じるか、`.env` の `SMART_LEDGER_PORT` を別の番号に変えてください。「ポート 5000 を使用できません(予約済み、権限が無い、または別のプログラムが使用中)」の場合は OS がそのポートを予約しているか、別のプログラムが 127.0.0.1 以外のアドレスで使っているので、ポート番号を変えてください
...
- 既定(`SMART_LEDGER_LAN` 未設定・`0`)は今までどおり `127.0.0.1` だけで待ち受け、PC 自身からしか開けません。`1` のときだけ `0.0.0.0` で待ち受けます。コンソールにはスマホ用の URL(`http://192.168.x.x:5000/` など)を 1 行表示します
...
- PIN を 5 回続けて間違えると PIN を作り直し、しばらく(30 秒。正しい PIN が入力されるまでは作り直すたびに倍、最長 2 時間)入力を受け付けません。正しい PIN が入力されると間違いの回数と一時停止の秒数は元に戻ります。「スマホで開く」画面は 15 秒ごとに更新され、新しい PIN を表示します
...
- スマホから開けないときは、PC とスマホが同じ Wi-Fi(ゲスト用 SSID ではない)につながっているか、Windows ファイアウォールの「許可されたアプリ」で Python の「プライベート」にチェックがあるかを確認してください。スマホ用の URL は起動時の PC の IP アドレスで決まり、それ以外の名前(PC 名など)で開くと 400 Bad Request になります。Wi-Fi に入り直すなどで IP アドレスが変わったら Smart Ledger を再起動してください
- VPN に接続していると、QR コードに VPN 側の IP アドレスが出てスマホから開けないことがあります。その場合は `ipconfig` で Wi-Fi の IPv4 アドレスを確かめ、`.env` の `SMART_LEDGER_LAN_ADDRESS` に設定して再起動してください
...
- スマホからの LAN 接続(localhost は PIN 不要、それ以外は PIN 入力画面へ、5 回続けての間違いで PIN の作り直しと一時停止、「スマホで開く」画面は localhost だけ、LAN モードと FLASK_DEBUG の併用拒否、Host ヘッダーの制限)
