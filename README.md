...
| 明細一覧 | 利用日・加盟店・金額・カテゴリ・分類元(rule / jev / manual / error)・confidence。月・収支(支出 / 収入)・カテゴリ・分類元・加盟店名・確信度(以上 / 以下 / 未設定)で絞り込み。内訳のある明細は下に内訳行(カテゴリ・金額・メモ)を表示(スマホはタップで開閉。ダッシュボードの最近の明細も同じ)。「手動で追加」から現金支出など CSV に無い明細を登録できる |
...
- **「Smart Ledger を起動します」の後、何分も反応がない**: `\\wsl.localhost\...` など WSL やネットワーク上のフォルダから起動すると、仮想環境（`.venv-windows`）のライブラリ読み込みがファイル共有越しになり、起動に 1〜3 分かかります。Windows のローカルディスクに `git clone` して `setup.cmd` → `start.cmd` を実行してください
...
現金での支出や、CSV を取り込んでいないカードの支出は、**明細一覧** の「手動で追加」から登録します
(利用日・加盟店 / 内容・金額・カテゴリ・収支・カード / 口座・メモ。カード / 口座の初期値は「現金」)。

- 手動明細は `import_id=manual`、`row_key` 空、`classification_source=manual` で `transactions` に保存し、`imports` シートには載せません
- 集計は取り込んだ明細と同じ扱いです。カテゴリを手動で付けた明細と同じく、要確認には出ず、後から登録したルールでも上書きしません(明細編集からルール登録はできます)
- CSV 取込の重複判定の対象外です。手動で登録したカードの CSV を後から取り込むと同じ支出が二重に計上されるので、取り込む前にその手動明細を削除してください(取込プレビューは、利用日・金額・収支が同じ手動明細がある新規行を警告します。取込からは除外しません)
- 金額は 0 以外の整数です。収入も正の金額で入力して収支で「収入」を選び、返金はマイナスの支出として入力します
- 取込の取り消しの対象外です。消すときは明細編集の「この明細を削除」を使います(内訳も一緒に削除します)
- 削除できるのは手動明細だけです。CSV から取り込んだ明細は、取込画面の取込履歴から取込ごと取り消します
...
- `import_id` は CSV 取込なら `imports` の取込 ID、画面から手動で追加した明細なら `manual`(`row_key` は空)
- `classification_source` は `rule` / `jev` / `manual` / `error`。`confidence` は `jev` のときのみ値が入ります(明細一覧の確信度の絞り込みは一覧と同じ小数 2 桁の表示値で比較し、数値を入れたときは空の明細を除きます。空の明細だけを見るときは「未設定」を選びます)
...
- 毎月同じように分ける明細は、内訳欄の **前回の内訳を複写** で、同じ加盟店キー(請求月などを除いた加盟店名。大文字小文字は無視)で収支と金額の符号(返金かどうか)が同じ、利用日がこの明細以前の、内訳を保存した直近の明細から内訳の行(カテゴリ・金額・メモ)を入力欄に写せます。金額が前回と違う場合は差額を最後の行に寄せます(最後の行が 0 円や明細金額と逆の符号になったときは表示で知らせるので、金額を直してください)。「内訳を保存」を押すまで保存されません
- 大きい項目だけ金額を入れ、端数は金額欄を空にしてカテゴリだけ選んだ行にまとめられます。保存時に明細金額 − 他の行の合計(残額)がその行に入ります。入力中は合計の横に「残り N 円 → カテゴリ」と表示し、保存後のメッセージにも入れた残額とカテゴリを出すので、他の行の入力ミスで残額がずれていないか確認してください。金額が空の行は 1 行だけで、残額が 0 円や明細金額と逆の符号になる場合は保存できません
- 保存がエラーになったとき(合計の不一致、残額が 0 円・逆の符号、カテゴリ未選択など)は、エラー文言とともに入力した行を残したまま編集画面を表示するので、そのまま直して保存し直せます
...
- 明細一覧の確信度の絞り込み(表示値での比較、空の除外と「未設定」、不正な入力の無視、ほかの絞り込みとの AND)
- allocations の二重計上防止・合計チェック、金額が空の行への残額の入力(0 円・逆符号・空の行 2 行以上の拒否)、一覧での内訳行の表示と内訳のカテゴリでの絞り込み(合計は明細金額)、前回の内訳の複写(複写元の選び方・差額の最後の行への寄せ・保存しないこと)、保存エラー時に入力した行を残すこと
- 手動明細の追加(入力の検証、集計への反映、取込の取り消しで消えないこと)と削除(内訳ごと。取込明細は拒否)
